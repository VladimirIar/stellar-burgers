...
>(({ title, titleRef, ingredients, ingredientsCounters }, ref) => {
  const getCategory = (title: string) => {
    if (title == 'Булки') return 'buns-category';
    if (title == 'Начинки') return 'mains-category';
    if (title == 'Соусы') return 'sauses-category';
  };
  return (
    <>
      ...
      <ul className={styles.items} ref={ref} data-testid={getCategory(title)}>
