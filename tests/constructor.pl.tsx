...
test.describe("Тестирование работы конструктора", () => {
  test('Добавление ингридиентов в конструктор', async({page}) => {
    await page.routeFromHAR('./tests/hars/data.har', {
      url: '**/api/ingredients',
      update: false
    });
  ...
  test('Работа модального окна', async({page}) => {
    ...
    await expect(modalContent.locator('h3')).toContainText('Краторная булка N-200i')
  ...
  test("Заркытие модального окна при клике в свободную область", async ({page}) => {
        await page.routeFromHAR('./tests/hars/data.har', {
      url: '**/api/ingredients',
      update: false,
    });
  ...
  test('Создание заказа', async({page}) => {
    ...
    await (page.getByTestId('constructor-section').getByRole('button', {name: 'Оформить заказ'})).click();
    const modal = page.getByTestId('modal')
    await expect(modal).toBeVisible();
